...
* Feature: New "Area based" algorithm for placing civilizations' starting Settlers, as an alternative to the original placement logic.
  ...
  * As a last resort, both algorithms place the Settlers on any other free land tile, so a civilization is not left without units while usable land exists. Tiles near the poles are only used once no other tile is available. Only a map without any free, non-excluded land tile leaves a civilization without a starting Settlers unit, instead of aborting the whole game setup.
