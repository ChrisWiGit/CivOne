...
namespace CivOne.UnitTests
{
	...
	internal sealed class MockedIRandomService(int fixedValue = 0, bool hit = false) : IRandomService
	{
		...
		public int NextInt(int maxExclusive) => maxExclusive <= 0 ? 0 : System.Math.Min(fixedValue, maxExclusive - 1);
		...
		public int NextInt(int min, int maxExclusive) => maxExclusive <= min ? min : System.Math.Clamp(fixedValue, min, maxExclusive - 1);
