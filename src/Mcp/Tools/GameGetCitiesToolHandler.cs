...
{
	public sealed class GameGetCitiesToolHandler : IMcpToolHandler
	{
		...
		public string Method => "game_get_cities";

		public ToolDefinition Definition => new(
			"game_get_cities",
			"Returns city data across all players or filtered by playerId/cityId and optional keys.",
			new
			{
				type = "object",
				properties = new
				{
					playerId = new { type = "integer", description = "Optional owner player id (0-based)." },
					cityId = new { type = "string", description = "Optional city id (GUID)." },
					keys = new
					{
						type = "array",
		...
		public McpResponse Handle(McpRequest request)
		{
			...
			IEnumerable<CityDto> cities = playerId.HasValue
				? (players[playerId.Value].Cities ?? [])
				: players.SelectMany(player => player.Cities ?? []);

			if (cityId.HasValue)
				cities = cities.Where(city => city.Id == cityId.Value);

			if (keys.Length == 0)
				return JsonResponse(request.Id, BuildSuccessPayload(cities));

			List<object> projected = [];
			HashSet<string> invalidKeys = new(StringComparer.OrdinalIgnoreCase);
			foreach (CityDto city in cities)
				projected.Add(ProjectByKeys(city, keys, invalidKeys));
		...
		public GameGetCitiesToolHandler(
